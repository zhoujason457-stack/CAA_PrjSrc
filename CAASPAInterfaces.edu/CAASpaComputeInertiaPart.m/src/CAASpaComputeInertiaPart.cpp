...
//  ---------
//
//  Batch program which computes the inertia of subassemblies of a product in a *.CATPart document
//
//  Illustrates :
...
//  ------
//
//  Type        : CAASpaComputeInertiaPart inputDirectory inputFile.CATPart
//
//  Inputs      :
//     inputDirectory        : directory containing the file
//     inputFile.CATPart     : name of the file
//
//  Return Code :
//     0 : OK
...
//============================================================================================
// 1 - Check arguments
// 2 - Load input *.CATPart and make it ready
// 3 - Search the Part
...
int main(int iArgc, char ** iArgv)
{
  ...
  if(3 != iArgc) 
  {
    cout << "Usage: CAASpaComputeInertia inputDirectory inputFile.CATPart"<< endl;
    _returnCode = 1;
    return _returnCode;
  }

  CATString inputName(iArgv[2]);
  CATString suffix = inputName.SubString(inputName.GetLengthInChar()-8,8);
  if (0 != suffix.compare(".CATPart")) 
  {
    cout << "inputFile is not a CATPart"<< endl;
    _returnCode = 1;
    return _returnCode;
  }
  ...
  if (SUCCEEDED(HR_Init))
  {
      ...
      if (SUCCEEDED(HR)) 
      {
          ...
          if (NULL != pRootList) 
          {
              ...
              {
                  CATIPrtPart_var hPart = piRootProduct->GetShapeRep();
                  ...
                  if (NULL_var != hPart)
                  {
                      ...
                      HR = hPart->QueryInterface(IID_CATIInertia,(void**)&piInertiaOnPart);
                      ...
                      if (SUCCEEDED(HR)) 
                      {
                          ...
                          HR = piInertiaOnPart->GetInertia (&density,&mass, position, matrix, components, values);
                          if (SUCCEEDED(HR)) 
                          {
                              ...
                              HR = hPart->QueryInterface(IID_CATIAlias,(void**)&piAliasOnProduct);
                              if (SUCCEEDED(HR)) 
                              {
                          ...
                          piInertiaOnPart->Release(); 
                          piInertiaOnPart = NULL;
                      }
                  }
