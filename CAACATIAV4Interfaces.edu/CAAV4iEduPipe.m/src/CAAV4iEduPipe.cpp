...
/*
  This batch is a sample to use Pipe CAA functions (PIBROC and PIBRTE) of CATIAV4Interfaces.
  ...
  2.3. retrieve the first element of the set
	2.4. retrive the connector origin for element ( returns -1 if not a valid element)
	2.5. retrive end type of the connector ( returns -1 if not a valid element)
  2.6. get the following elements
  2.7. get the following sets
  2.8. get the following workspaces
*/
int main(int nbarg, char ** argv)
{
	...
	if (! CATV4iGetMaster(doc, wspElem, ier) )
	{
					...
					if (! CATV4iGisels(setElem, NULL, element, endEls, ier) )
					{
						while(!endEls && !ier)
						{
							...
							if (! CATV4iGisels(setElem, element, nextElement, endEls, ier) )
							{
								if (element)
								{
									int retcode = CATV4iPibroc(element, orgin, ier1);
									if(retcode == 0)
									{
										flag = 1 ;
										outputFile << "Valid pipe connector element ==> " << orgin[0] << " , " << orgin[1] << " , " << orgin[2] << " , " << "errorcode= " <<ier1 <<endl;
									}									
									retcode = CATV4iPibrte(element,typext , ier1);
									if(retcode == 0)
									{
										flag = 1 ;
										outputFile << "Valid pipe element : end type of connectors : typext ==" 
											<<typext << " , " << "errorcode= " <<ier1 << endl;
		...
		{
			outputFile << "<== CATV4iEduPipe " << endl;
			outputFile << "Valid pipe element / connector not found in the model" << " , " << "errorcode = " << -1 << endl;
			result = 1;
		}
	}
	else
