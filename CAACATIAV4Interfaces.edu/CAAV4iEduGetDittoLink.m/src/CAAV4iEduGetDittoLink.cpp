...
/*
  This batch is a sample to use the API CATV4iTstGetDittoLink of CATIAV4Interfaces
  ...
  5. Retrieve the ditto element linked to the above connector
*/
int main(int nbarg, char ** argv)
{
	...
	outputFile << "==> CATV4iTstGetDittoLink " << endl;
							...
							if (! CATV4iGisels(setElem, element, nextElement, endEls, ier) )
							{
								if (element)
								{
									...
									if (itp == 61)
									{
										...
										int retcode = CATV4iGetDittoLink(element, oElement, ier1);										 
										if ((retcode == 0)&&(oElement != NULL))
										{
											flag = 1 ;
											outputFile << "Valid connector ==> " << element->GetId() << " DITTO element ==> " << oElement->GetId() << " , " << "errorcode = " <<ier1 <<endl;
										}			
									}
									element->Release();
									element=NULL;
