...
/*
  This batch is a sample to use the API CATV4iGetMechLink of CATIAV4Interfaces
  0. Open the output file
  1. Open the model passed in argument
  2. Scan the model
  3. Scan all workspaces, sets and elements in the model
  4. Retrieve the connector OR connector occurrence in the model
  5. Retrieve the connector OR connector occurrence it is linked with above
*/
int main(int nbarg, char ** argv)
{
	...
	outputFile << "==> CATV4iTstGetMechLink " << endl;
	...
	if (! CATV4iGetMaster(doc, wspElem, ier) )
	{
							...
							if (! CATV4iGisels(setElem, element, nextElement, endEls, ier) )
							{
								if (element)
								{
									...
									if (itp == 60 || itp == 61)
									{
										...
										int retcode = CATV4iGetMechLink(element, oElement, ier1);										 
										if ((retcode == 0)&&(oElement != NULL))
										{
											flag = 1 ;
											outputFile << "Valid connector / connector occurrence ==> " << element->GetId() << " Mechanically Linked to ==> " << oElement->GetId() << " , " << "errorcode = " <<ier1 <<endl;
										}			
									}
									element->Release();
									element=NULL;
		...
		{
			outputFile << "<== CATV4iTstGetMechLink " << endl;
			outputFile << "Valid connector / connector occurrence not found in the model" << " , " << "errorcode = " << -1 << endl;
			result = 1;
		}
	}
	else
