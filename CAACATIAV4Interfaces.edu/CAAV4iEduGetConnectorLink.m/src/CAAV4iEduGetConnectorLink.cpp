...
/*
  This batch is a sample to use the API CATV4iGetConnectorLink of CATIAV4Interfaces (PIBRCP).
  ...
  5. Retrieve the parent connector linked to the above connector occurrence
*/
int main(int nbarg, char ** argv)
{
	...
	outputFile << "==> CATV4iTstGetConnectorLink " << endl;
							...
							if (! CATV4iGisels(setElem, element, nextElement, endEls, ier) )
							{
								if (element)
								{
									...
									if (itp == 61)
									{
										...
										int retcode = CATV4iGetConnectorLink(element, oElement, ier1);										 
										if ((retcode == 0)&&(oElement != NULL))
										{
											flag = 1 ;
											outputFile << "Valid connector occurrence ==> " << element->GetId() << " Parent CNP ==> " << oElement->GetId() << " , " << "errorcode = " <<ier1 <<endl;
										}			
									}
									element->Release();
									element=NULL;
