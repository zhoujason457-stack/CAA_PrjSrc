...
/*
  This batch is a sample to use the API CATV4iGetPipingLink of CATIAV4Interfaces
  0. Open the output file
  1. Open the model passed in argument
  2. Scan the model
  3. Scan all workspaces, sets and elements in the model
  4. Retrieve the connector in the model
  5. Retrieve the pipe element linked to the above connector
*/
int main(int nbarg, char ** argv)
{
	...
	outputFile << "==> CATV4iTstGetPipingLink " << endl;
	...
	if (! CATV4iGetMaster(doc, wspElem, ier) )
	{
							...
							if (! CATV4iGisels(setElem, element, nextElement, endEls, ier) )
							{
								if (element)
								{
									...
									if (itp == 60)
									{
										...
										int retcode = CATV4iGetPipingLink(element, oElement, ier1);										 
										if ((retcode == 0)&&(oElement != NULL))
										{
											flag = 1 ;
											outputFile << "Valid connector ==> " << element->GetId() << " PIPE element ==> " << oElement->GetId() << " , " << "errorcode = " <<ier1 <<endl;
										}			
									}
									element->Release();
									element=NULL;
		...
		{
			outputFile << "<== CATV4iTstGetPipingLink " << endl;
			outputFile << "Valid connector not found in the model" << " , " << "errorcode = " << -1 << endl;
			result = 1;
		}
	}
	else
