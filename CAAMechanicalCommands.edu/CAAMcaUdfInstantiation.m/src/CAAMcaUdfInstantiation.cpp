...
//==========================================================================
//
//      Abstract: This use case shows how to instantiate a User Feature
//
//            * Each instantiation will be done in the CAAUdfModel.CATPart 
//              which is located in CAAMechanicalCommands.edu/InputData. 
//              This document contains at the beginning 3 points:
//             
//            -Part1
//              - xy plane
//              - yz plane
...
//                - Point.3 
//
//            * The User Feature reference is in another document:   
//              CAAUdfLoft.CATPart also located in CAAMechanicalCommands/InputData.
//
...
//                   - Radius of the circle through the first input 
//                   - Radius 
//              - PartBody 
//              - Inputs
//              - LoftWith2Circles
//
//
//            After the execution of this use case the CAAUdfModel.CATPart document
//            contains 2 instantiations of CAAUserFeatureSample. This Part is saved
//            as CAAUdfModelWithInstances.CATPart :
//              
//
//            -Part1
//              - xy plane
...
//                - Point.2
//                - Point.3
//                - CAAUserFeatureSample.1  ( First instantiation )
//                - TheLoftWithPoint2and3   ( Second instantiation )                 
//
//
//  Steps :
//    1- Creates a session
//    2- Opens CAAUdfLoft and CAAUdfModel documents
//    3- Retrieves the root container of each Part document 
//    4- Retrieves the User Feature reference in the CAAUdfLoft document
//       4-1 Retrieves a CATIUdfFactory interface pointer of the root container
//       4-2 Retrieves the User Feature reference in the User Feature Set
//       4-3 Retrieves, for information, the role of each input 
//    5- First Instantiation
//       5-1 Sets the destination path
//       5-2 Gives input 
//          5-2-1 Sets new inputs taken from CAAUdfModel
//          5-2-2 Checks new inputs
//       5-3 Instantiates in the document 
//       5-4 Retrieves the new instance pointer 
//       5-5 Ends the current instantiation
//    6- Second Instantiation
//       6-1 Sets the destination path
//       6-2 Gives inputs 
//          6-2-1 Sets the second input thanks to UseIdenticalName 
//          6-2-2 Sets the first input thanks to SetNewInput
//          6-2-3 Checks new inputs
//       6-3 Changes parameters values 
//       6-4 Instantiates in the document 
//       6-5 Changes the display name
//       6-6 Retrieves the new instance pointer 
//       6-7 Ends the current instantiation
//    7- Ends the instantiations
//    8- Updates the new instances
//    9- Saves CAAUdfModel as CAAUdfModelWithInstances
//    10- Closes documents
//    10- Deletes the session
//
//
//==========================================================================
// How to execute :
//
//   CAAMcaUdfInstantiation InputPath [OutputPath]
//
//   with     InputPath   : $WSROOT/CAAMechanicalCommands.edu/InputData
//            OutputPath  : directory to save the Part with the two instantiations
//
// Return code :
...
// Local Framework
#include "CAAMcaGetGeometry.h"          // To retrieve inputs from CAAUdfModel
...

{
  ...
  {
     cout << endl;
     cout << "CAAMcaUdfInstantiation InputPath  [OutputPath]"<< endl;
     cout << "with : InputPath        : $WSROOT/CAAMechanicalCommands.edu/InputData"<< endl;
     cout <<"        OutputPath: Directory to save as the document with two User Features"<<endl;
     cout << endl;
     return 1;
  }

  cout << "The CAAMcaUdfInstantiation main program begins ...."<< endl << endl;
  ...
  //----------------------------------------------------
  //2- Opens the CAAUdfModel and the CAAUdfLoft models 
  //-----------------------------------------------------

  // The input directory 
  CATString InputData = iArgv[1] ;

  // document into which the User Feature reference will be instantiated
  ...
  // document which contains the User Feature reference
  ...
  // Retrieves the Mechanical Part of the CAAUdfModel
  ...
  // 4-1 Retrieves CATIUdfFactory interface pointer to the root container
  // --------------------------------------------------------------------
  ...
  // 5-2-2 Checks and displays the new inputs 
  // ----------------------------------------
  ...
  // Use  CATISpecObject in batch mode. 
  // In interactive mode, use the 
  // CATPrtUpdateCom class. A use case in Mechanical Modeler 
  // explains its usage. "Editing a Combined Curve"
  ...
  CATString DocumentSaveName = "CAAUdfModelWithInstances.CATPart" ;
  if ( 3 == iArgc )
  {
  ...
  if (S_OK == rc)
  {
     cout << "   The "<< DocumentSaveName.CastToCharPtr() << " document with the 2 instantiations is saved" << endl ;
  }
  else
  {
  ...
  // The Unlock makes the deletion of the document
  // the CATDocumentServices::Remove is not necessary 
  //
  rc = ::CATUnLockDocument(*pCAAUdfLoft);
  if (FAILED(rc))
  {
     cout << "ERROR in closing document "<< CAAUdfLoftDocName.CastToCharPtr() << endl ;
     return 1;
  }
  pCAAUdfLoft = NULL ;
  ...
  cout << endl;
  cout << "The CAAMcaUdfInstantiation  main program is ended."<< endl << endl;
