...
CATUnicodeString SplitNameFromPath(CATUnicodeString& iPath,CATUnicodeString& Dir)
{
   ...
   if (Length != -1)
   {
      ...
      Dir = iPath.SubString(0,Length);
   }
   else rc=iPath;
   //Remove extension
   int idxPoint = rc.SearchSubString(".", 0, CATUnicodeString::CATSearchModeBackward);
   rc = rc.SubString(0, idxPoint);
   return rc;
}


//------------------------------------------------------------------------
CAAxPDMCheckSessionPanel::CAAxPDMCheckSessionPanel(CATDialog* parent)
:CATDlgDialog(parent,"CheckSession",CATDlgWndOK|CATDlgWndCANCEL|CATDlgWndHELP|CATDlgWndTitleBarHelp)
{
   BuildPanel();
   _oItemsWithError=NULL;
}
//------------------------------------------------------------------------
CAAxPDMCheckSessionPanel::~CAAxPDMCheckSessionPanel()
{
   _list=NULL;
   _errorhelp=NULL;
...
//------------------------------------------------------------------------
void CAAxPDMCheckSessionPanel::BuildPanel()
{
   _list = new CATDlgMultiList(this,"List1");
   if (NULL==_list) return;
   CATUnicodeString aTitle;
   CATUnicodeString multiListTitles[6]; 
   multiListTitles[0] = CATMsgCatalog::BuildMessage("CAAxPDMCheckSessionPanel","multiListTitles0",NULL,0,"Item");
   multiListTitles[1] = CATMsgCatalog::BuildMessage("CAAxPDMCheckSessionPanel","multiListTitles1",NULL,0,"Type");
   multiListTitles[2] = CATMsgCatalog::BuildMessage("CAAxPDMCheckSessionPanel","multiListTitles2",NULL,0,"Error");
   multiListTitles[3] = CATMsgCatalog::BuildMessage("CAAxPDMCheckSessionPanel","multiListTitles3",NULL,0,"Object");
   multiListTitles[4] = CATMsgCatalog::BuildMessage("CAAxPDMCheckSessionPanel","multiListTitles4",NULL,0,"Description");
   multiListTitles[5] = CATMsgCatalog::BuildMessage("CAAxPDMCheckSessionPanel","multiListTitles5",NULL,0,"Path");
   ...
   AddAnalyseNotificationCB (_list,_list->GetListSelectNotification(),
	   (CATCommandMethod) &CAAxPDMCheckSessionPanel::SelectLineCB,NULL);
   AddAnalyseNotificationCB (_list,_list->GetListActivateNotification(),
	   (CATCommandMethod) &CAAxPDMCheckSessionPanel::ActivateLineCB,NULL);
...
//------------------------------------------------------------------------
void CAAxPDMCheckSessionPanel::SetListInfo(const CATListOfCATUnicodeString& ListName,const CATListOfCATUnicodeString& ListError,
...
void CAAxPDMCheckSessionPanel::SelectLineCB (CATCommand* iCmd, CATNotification* iNotif, CATCommandClientData iData)
{
   if (_list && _errorhelp)
   {
...
void CAAxPDMCheckSessionPanel::ActivateLineCB (CATCommand* iCmd, CATNotification* iNotif, CATCommandClientData iData)
{
