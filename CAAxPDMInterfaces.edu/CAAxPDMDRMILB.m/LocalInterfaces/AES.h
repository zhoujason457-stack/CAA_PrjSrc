...
//
// Used as sample for test purpose in order to have an AES algorithm
// Do not use outside of test FW
// The following code is public domain but it should remain in test FW !!!
...
public:
	...
	// The input array length must be a multiple of 16 bytes, 
	// the remaining part is DISCARDED.
	// so it actually encrypts inputLen / 128 blocks of input and puts it 
	// in outBuffer 
	...
	// Encrypts the input array (can be binary data)
	// The input array can be any length , it is automatically padded on 
	// a 16 byte boundary.
