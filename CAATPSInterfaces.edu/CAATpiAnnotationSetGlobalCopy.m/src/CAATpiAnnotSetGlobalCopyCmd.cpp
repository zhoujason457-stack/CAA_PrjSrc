...
#include "CAATpiAnnotSetGlobalCopyCmd.h"
...
CATCreateClass (CAATpiAnnotSetGlobalCopyCmd);

//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
CAATpiAnnotSetGlobalCopyCmd::CAATpiAnnotSetGlobalCopyCmd () : CATStateCommand ("CAATpiAnnotSetGlobalCopyCmd", CATCommandModeExclusive)
...
// Destructor
//-----------------------------------------------------------------------------
CAATpiAnnotSetGlobalCopyCmd::~CAATpiAnnotSetGlobalCopyCmd ()
{
...
// BuildGraph
//-----------------------------------------------------------------------------
void CAATpiAnnotSetGlobalCopyCmd::BuildGraph ()
{
  ...
  if(pSelectState)
  {
    ...
    // Define transitions
    AddTransition(pSelectState, NULL, IsOutputSetCondition(_pAgentAnnotationSet), Action((ActionMethod)&CAATpiAnnotSetGlobalCopyCmd::LaunchGlobalCopy));
  }
}
...
//-----------------------------------------------------------------------------
boolean CAATpiAnnotSetGlobalCopyCmd::LaunchGlobalCopy(void * ipData)
{
  ...
  // Geometries of first part are symmetrical from the geometries of the second part.
  // The symmetry plane is included into the second part (parallel to Plane XZ through Point(0, 20, 20))
  ...
  // in this sample the symmetry plane is parallel to Plane XZ through Point(0, 20, 20)
  // *********
  CATMathPoint *pPoint = new CATMathPoint(0.0, 20.0, 0.0);
  CATMathVector *pVector_I = new CATMathVector(1.0, 0.0, 0.0);
  CATMathVector *pVector_II = new CATMathVector(0.0, 0.0, 1.0);

  CATMathPlane *pSymmetryPlane = new CATMathPlane();
  if(!pSymmetryPlane) return TRUE;
  pSymmetryPlane->Set(*pPoint, *pVector_I, *pVector_II);
  pTransfo = new CATMathTransformation(*pSymmetryPlane);
  ...
  if (pSymmetryPlane){delete pSymmetryPlane; pSymmetryPlane =0; }
  if (pPoint){delete pPoint; pPoint =0; }
  if (pVector_I){delete pVector_I; pVector_I =0; }
  if (pVector_II){delete pVector_II; pVector_II =0; }

  return TRUE;
}

void CAATpiAnnotSetGlobalCopyCmd::LaunchWarningPanel()
{
  // Send warning panel
