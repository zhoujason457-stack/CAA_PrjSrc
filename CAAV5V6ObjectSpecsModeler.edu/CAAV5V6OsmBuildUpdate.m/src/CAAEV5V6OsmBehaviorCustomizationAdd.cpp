...
// --------------------------------------------------------------------------------------------------
#include "CAAEV5V6OsmBehaviorCustomizationAdd.h"               
...
CATImplementClass(CAAEV5V6OsmBehaviorCustomizationAdd, 
				  DataExtension,
                  CATIFmFeatureBehaviorCustomization,
				  CAAOsmAdd);
CATImplementBOA(CATIFmFeatureBehaviorCustomization,CAAEV5V6OsmBehaviorCustomizationAdd); 

//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
CAAEV5V6OsmBehaviorCustomizationAdd::CAAEV5V6OsmBehaviorCustomizationAdd()
{
}
...
{
	cout << "   Starting CAAEV5V6OsmBehaviorCustomizationAdd::Build" << endl << flush;
	...
	//--------------------------------------------------
	// 3-1- Create a facade bound to the "CAAOsmAdd" feature (this)
	CATFmFeatureFacade FmFeatFacadeOnCAAEV5V6OsmBehaviorCustomizationAdd(MyCredential,this);

	// 3-2- Retrieve the value of the "First" attribute
	int iFirstAttr = 0;
	CATFmAttributeValue FmAttrValue;
	HRESULT rc =  FmFeatFacadeOnCAAEV5V6OsmBehaviorCustomizationAdd.GetValue(AddFirstAttrKey,FmAttrValue);
	if (SUCCEEDED(rc))
	...
	if (SUCCEEDED(rc))
	{
		rc =  FmFeatFacadeOnCAAEV5V6OsmBehaviorCustomizationAdd.GetValue(AddSecndAttrKey,FmAttrValue);
		if (SUCCEEDED(rc))
	...
	if (SUCCEEDED(rc))
	{
		...
		rc =  FmFeatFacadeOnCAAEV5V6OsmBehaviorCustomizationAdd.SetValue(AddSumAttrKey,FmAttrValue);
		// 5-2- Retrieve the value of the attribute "Sum". Confirm if it is set to the new value
		if (SUCCEEDED(rc))
		{
			rc =  FmFeatFacadeOnCAAEV5V6OsmBehaviorCustomizationAdd.GetValue(AddSumAttrKey,FmAttrValue);
			if (SUCCEEDED(rc))
			{
				int iSumValue = 0;
