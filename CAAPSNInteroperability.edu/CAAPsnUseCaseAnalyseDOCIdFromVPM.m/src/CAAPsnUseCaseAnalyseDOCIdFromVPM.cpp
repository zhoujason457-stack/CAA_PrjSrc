...
// How to execute :
// 
//   mkrun -c "CAAPsnUseCaseAnalyseDOCIdFromVPM Coid  hostName user pwd role server "  
//e.g.         CAAPsnUseCaseAnalyseDOCIdFromVPM 41419be22d212fa4 bldsunkarnaplb ""  "" VPMADMIN SAMPLE
//          
// Return code :
...
int main(int iArgc, char *argv[])        
{
  ...
  if ((7 != iArgc) )
    {
      cout << endl;
      cout << "CAAPsnUseCaseAnalyseDOCIdFromVPM.cpp Coid hostName User Passwd Role Server"<< endl;
  ...
  cout << "The CAAPsnUseCaseAnalyseDOCIdFromVPM.cpp main program begins ...."<< endl << endl<<flush;
  ...
  if(pDocRoot!=NULL)
  pDocRoot->GetDocId(&piDocId);
  
  rc=CATPDMServices::CATAnalyseDocIdFromVPM (piDocId,oCoid,oCompid,oCaenv,oCatab);
    
	//----------------------------------------------------------------
	//Expected PrintableId From The extracted Parameters
	//-----------------------------------------------------------------
  expectedPrintableId.Append(oCoid);
  expectedPrintableId.Append(oCompid);
  expectedPrintableId.Append(oCaenv);
  expectedPrintableId.Append(oCatab); 
  cout<<"PrintableId Derieved From Parameters="<<expectedPrintableId.ConvertToChar()<<endl;
   
  //--------------------------------------------------------------------
  // 7 ii)-Check the PrintableId got from product and the one expected are equal
  //---------------------------------------------------------------------
  if(0 == printableId.Compare(expectedPrintableId))
    {
      cout<<"Derieved PrintableId is not correct"<<endl;
      // PSNV5_TRACE("Derieved PrintableId is not correct");
      rc=E_FAIL;
    }
  ...
  cout <<"The CAAPsnUseCaseAnalyseDOCIdFromVPM.cpp main program is ended."<< endl << endl;
